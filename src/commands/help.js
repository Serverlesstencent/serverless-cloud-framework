...
${command('param list')}       在项目目录下展示已配置的动态参数`;
  ...
  cli.log(
    `
${title('快速开始')}
${gray('* 直接输入 "serverless-cloud-framework" (或缩写 "scf") 进行项目初始化')}
...
${title('命令')}
${gray('* 您可以通过 "serverless-cloud-framework" 或简称 "scf" 来执行命令')}
${gray('* 使用 "scf [command] --help" 获取详细帮助信息')}
...
${command2(
  'param set'
)}               在项目中配置参数,支持多参数配置: scf param set key1=value1 key2=value2
`,
    'param list': `
  ...
  if (allowedCommands[commandName]) {
    ...
    cli.log(
      `Serverless: ${chalk.yellow(
        `没有找到 "${commandName}" 命令. 你想查看的是 "${suggestion}" 的帮助信息吗? 可以通过 "scf help" 查看所有可用命令`
      )}`
    );
    cli.log();
  }
