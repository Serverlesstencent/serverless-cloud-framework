...
// slt-binary-sv-1300963013.file.myqcloud.com/latest-tag 注释掉的
const resolveLatestTag = async () => {
  const { body } = await got.get('https://scf-binary-sv-1300862921.cos.ap-guangzhou.myqcloud.com/latest-tag');
  return body;
};
...
const resolveUrl = (tagName) => {
  ...
  return `https://scf-binary-sv-1300862921.cos.ap-guangzhou.myqcloud.com/${tagName}/serverless-tencent-${platform}-${arch}${
    process.platform === 'win32' ? '.exe' : ''
  }`;
};
